"""
Started with a straight up copy of the bluepy sensortag.py
example file.  I've updated some of the calculations for the sensors
present for the CC2650 SensorTag, and added the Lux sensor.
Some like the Microphone don't appear to be revealed through GATT.
"""
...
class HumiditySensor(SensorBase):
    ...
    def read(self):
        ...
        temp = ((rawT / 65536.0) * 165 - 40)
        RH = (((rawH & 0xFFFC)/65536.0)*100)
        #ipdb.set_trace()
        return (temp, RH)
...
class LuxometerSensor(SensorBase):
    ...
    def read(self):
        ...
        #ipdb.set_trace()
        mantissa = raw & 0x0FFF
        exponent = (raw >> 12) & 0xFF
        magnitude = pow(float(2.0), exponent)
...
class MicrophoneSensor(SensorBase):
    svcUUID  = _TI_UUID(0xAC00)
    dataUUID = _TI_UUID(0xAC01)
    ctrlUUID = _TI_UUID(0xAC02)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-P','--microphone', action='store_true', default=False)
    ...
    if arg.microphone or arg.all:
        tag.microphone.enable()
    ...
    while True:
       ...
       if arg.microphone or arg.all:
           print ("Microphone: ", tag.microphone.read())
